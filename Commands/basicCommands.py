...
def start(bot, update):
    ...
    if bd.user_id in bd.chat_id_authorized:
        bot.sendMessage(chat_id=bd.chat_id, text=ms.helpOrStartAdmin, reply_to_message_id=bd.message.message_id,
    ...
    if bd.user_id in bd.chat_id_authorized or bd.user_id == bd.chat_id:
        try:
            ...
            changelogTemp = ms.changelogMessage.replace(
                '$args1', "## [" + changelogTemp.split("## [")[2]).replace('$args2', "## [" + changelogTemp.split("## [")[1])
            ...
            for line in changelogTemp:
                ...
                if "##" in line:
                    line = line.replace("##", "*") + "*"
                changelogStr += line + "\n"
            bot.sendMessage(chat_id=bd.chat_id, text=changelogStr,
                            reply_to_message_id=bd.message.message_id, parse_mode="MARKDOWN")
        except Exception as e:
            bd.exceptionHandler(bot, update, __name__, e)
...
def contact(bot, update, args):
    ...
    if args is None or args == '' or args == []:
        ...
        bot.sendMessage(chat_id=bd.chat_id_authorized[0], text=ms.contactMessage.replace(
            '$args1', bd.username).replace('$args2', str(bd.user_id)).replace('$args3', ' '.join(args)))
