...
def birthdayList(bot, update, args=None):
    ...
    if bot.get_chat_member(chat_id=bd.chat_id_CoreDumped, user_id=bd.user_id).status in ['creator', 'administrator', 'member']:
        ...
        if(eventList is None or eventList == {} or eventList == [] or eventList == ""):
            ...
            for event in eventList:
                date = timezone('UTC').localize(datetime.strptime(
                    event['start']['date'], '%Y-%m-%d')).astimezone(timezone('Europe/Madrid')).strftime("%d-%m")
                if bd.user_id not in bd.chat_id_authorized and bot.get_chat_member(chat_id=bd.chat_id_CoreDumped, user_id=bd.user_id).status not in ['creator', 'administrator']:
                    formatedEventList += (str(lap) + '. ' + date + ': ' + event['summary'].split('|')[0] + '\n')
                else:
                    formatedEventList += (str(lap) + '. ' + date + '-' +
                                          event['summary'].split('|')[2] + ': ' + event['summary'].split('|')[0] + '\n')

                lap += 1
            if bd.user_id in bd.chat_id_authorized or bot.get_chat_member(chat_id=bd.chat_id_CoreDumped, user_id=bd.user_id).status in ['creator', 'administrator']:
...
def eventList(bot, update, args=None):
    ...
    if(eventList is None or eventList == {} or eventList == [] or eventList == ""):
        ...
        for event in eventList:
            date = timezone('UTC').localize(datetime.strptime(
                event['start']['dateTime'], '%Y-%m-%dT%H:%M:%SZ')).astimezone(timezone('Europe/Madrid')).strftime("%d-%m-%Y %H:%M")
...
def eventAdd(bot, update, args=None):
    ...
    if bd.user_id not in bd.chat_id_authorized and bot.get_chat_member(chat_id=bd.chat_id_CoreDumped, user_id=bd.user_id).status not in ['creator', 'administrator']:
        ...
        if(result is None or result == {} or result == [] or result == ""):
            ...
            date = timezone('UTC').localize(datetime.strptime(
                result['start']['dateTime'], '%Y-%m-%dT%H:%M:%SZ')).astimezone(timezone('Europe/Madrid')).strftime("%d-%m-%Y %H:%M")
            bot.sendMessage(chat_id=bd.chat_id, text=ms.newEventAdded.replace(
                "$args1", result['summary']).replace("$args2", date), reply_to_message_id=bd.message.message_id)
