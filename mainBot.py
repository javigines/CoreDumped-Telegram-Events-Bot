...
def listB(bot, update, args=None):
	...
	if birthdaylist is not None:
		if user_id == chatIDDeveloper:
			...
			birthdaylist = newbirthdayString
			if(chat_id != user_id):
				bot.sendMessage(chat_id=chat_id, text="Te he enviado el listado de cumpleaños por privado.")
...
start_handler = CommandHandler('startB', startB, pass_args=False)
dispatcher.add_handler(start_handler)
addB_handler = CommandHandler('birthday', addBirthday, pass_args=True)
dispatcher.add_handler(addB_handler)
remove_handler = CommandHandler('removeB', removeB, pass_args=True)
dispatcher.add_handler(remove_handler)
list_handler = CommandHandler('listB', listB, pass_args=True)
dispatcher.add_handler(list_handler)
restart_handler = CommandHandler('restartB', restartB, pass_args=False)
dispatcher.add_handler(restart_handler)
stop_handler = CommandHandler('stopB', stopB, pass_args=False)
dispatcher.add_handler(stop_handler)
leave_handler = CommandHandler('leaveB', leaveGroup, pass_args=False)
